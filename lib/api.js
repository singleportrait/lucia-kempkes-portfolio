...
function parseHomepage(entry) {
  const fields = entry?.items?.[0].fields;
  if (fields) {
    return {
      title: fields.title,
      projects: fields.projects,
      shareImage: fields.shareImage || null,
      metaDescription: fields.metaDescription || null,
    }
  }
}

function parseHomepageProjectSlugs(entry) {
  if (entry?.items && entry.items[0].fields.projects.length !== 0) {
    return entry.items[0].fields.projects.map(project => project.fields.slug);
  }
}
...
function parsePage({ fields }) {
  ...
  return {
    ...
    parentPageSlug: fields.parentPage?.fields.slug || null,
    shareImage: fields.horizontalPreviewImage || null,
    metaDescription: fields.metaDescription || null,
  }
}
...
export async function getHomepageProjectSlugs() {
  ...
  return parseHomepageProjectSlugs(entry);
}
